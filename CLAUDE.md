...
- **Protocole "chimie"** (`protocole.js`) : lecture seule, ne jamais écrire dans la feuille `Protocol` du client — la saisie (dates, molécules) reste manuelle côté coach dans Sheets, l'app recalcule juste l'affichage à la volée. Activable individuellement par client via le toggle 🧬 dans "Mes clients" (coach), colonne J de la feuille `Clients`.
